...
- **Stock tags**: `T` opens the Tags dialog; Generate (`Shift+Enter`) runs the claude CLI. While a run is going the row offers Stop (`Esc`), which kills the run and leaves the dialog standing with everything typed into it, and Background (`Ctrl+Enter`), which hides the dialog and lets the run finish; Background over an idle dialog starts a run and sends it away in the same press. A backgrounded run updates the cache and the tag overlay, writes what the autosave settings allow — the XMP sidecar of the photo (named after the RAW when there is a pair, after the image itself when there is none, so a JPEG that came without a RAW keeps its tags too) and the JPEG packet — and reports itself through a notification. With both autosaves off it writes nothing and leaves the tags in the cache and on the overlay, waiting for the Save button; a dialog opened on them afterwards is told they reached no file and writes them, rather than taking the cache for the sidecar. Tags a file already holds are not written to it again, so opening a dialog and closing it leaves both files alone. Save writes both files at once: the sidecar whatever the fields hold, and the JPEG packet only when the photo is a JPEG and the tags meet every stock requirement, since that file is the photo itself; a save that leaves the JPEG alone says why. A dialog that closes over a running generation — backgrounded, or clicked away — writes no sidecar of its own: it hands its fields to the run, which writes what it generated, or those fields when it generates nothing, so one writer owns the file. Pressing `T` on a photo whose run is still going re-attaches the dialog to it and puts the handed-over fields back on screen, so the run answers to a dialog again and writes nothing of its own. The sidecar travels only with the RAW: a copy that takes the RAW takes it along and waits for a run still going, while a copy of the image alone — and any copy of a photo that has no RAW — leaves the sidecar where it was. Deleting a photo whose run is still going kills the run and waits for it to let the files go, so no orphan sidecar is left behind. Every run is cancelled when the app exits, and one that was handed a dialog's fields and never landed writes them on the way out — unless the sidecar is saved by hand, when nothing was ever owed to a file. The Concept note is saved with the tags — `photo:Concept` in the sidecar and in the JPEG packet — so reopening the dialog shows what the tags were generated from; a note alone is worth a sidecar, and the EXIF fallback path has no field for it and says so. The Notes field carries standing information about the photo that the generation has to take into account beyond the concept it was asked for; it goes into the request last, so it may run to several lines, and it is never part of what the generation answers with. It is saved the same way — `photo:Notes` in the sidecar and in the JPEG packet — so a photo reopens on the notes it was last generated from; notes alone are worth a sidecar too, a generation carries them back with the tags it found, and the EXIF fallback path has no field for them either and says so. The Editorial mark and the day it names ride along the same way — `photo:Editorial` and `photo:EditorialDate` in the sidecar and in the JPEG packet — so a photo reopens ticked on the day it was marked for; a mark alone is worth a sidecar too, a generation carries it back with the tags it found, and the EXIF fallback path has no field for it either and says so. Every generation still in flight shows as a plate in the top-right corner of the photo — an icon, the time it has been running and the photo name, oldest on top — and the plate goes when the generation answers or is stopped. Tags travel from one photo to another through the dialog alone: `Alt+C` takes the title and the keywords on screen — the place, the concept, the notes and the editorial mark stay with the photo they were typed for — and `Alt+V` puts them into another photo's dialog, asking first when that dialog already holds tags. A paste writes no file of its own; what is pasted is written by the autosaves and by the Save button, the same as anything else typed there.
...
- **Settings**: `I` opens the Settings window, which applies and remembers every switch the moment it is ticked: the tag overlay, autosaving the XMP sidecar, autosaving the JPEG, and whether the Tags dialog offers its Save button. With both autosaves on there is nothing left for that button to do, so its switch greys out and the button leaves the dialog. Sort order and direction are remembered as well. The main window always opens maximized.
