...
- **Stock tags**: `T` opens the Tags dialog; Generate (`Shift+Enter`) runs the claude CLI. While a run is going the row offers Stop (`Esc`), which kills the run and leaves the dialog standing with everything typed into it, and Background (`Ctrl+Enter`), which hides the dialog and lets the run finish; Background over an idle dialog starts a run and sends it away in the same press. A backgrounded run updates the cache and the tag overlay, writes the XMP sidecar of the RAW pair (a JPEG without a RAW pair is never written to on its own — that stays behind Save JPEG) and reports itself through a notification. A dialog that closes over a running generation — backgrounded, or clicked away — writes no sidecar of its own: it hands its fields to the run, which writes what it generated, or those fields when it generates nothing, so one writer owns the file. Pressing `T` on a photo whose run is still going re-attaches the dialog to it. Copying a photo whose run is still going waits for it, so the sidecar goes with the copy; deleting one kills the run and waits for it to let the files go, so no orphan sidecar is left behind. Every run is cancelled when the app exits.
