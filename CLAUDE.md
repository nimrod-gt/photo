...
- **Settings**: `I` opens the Settings window, which applies and remembers every switch the moment it is ticked: the tag overlay, autosaving the XMP sidecar, autosaving the JPEG, and whether the Tags dialog offers its Save button. With both autosaves on there is nothing left for that button to do, so its switch greys out and the button leaves the dialog. Sort order and direction are remembered as well. The window always opens maximized.
